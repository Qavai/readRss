...
var Reader = function (rootEl) {
    ...
    if (1 || window.location.search.indexOf('fullscreen') != -1 || window.location.hostname == 'rss.loc') {
    ...
    s.push('<form>');
    s.push('<input type="text" name="search" placeholder="https://" value="" />');
    ...
    s.push('<div id="pop-menu-feed-edit" class="popup-menu dn">');
    s.push('<p>Edit feed url:</p>');
    s.push('<form>');
    s.push('<input type="text" name="feed_edit" placeholder="https://" value="" />');
    s.push('<button class="button_wait" name="submit">');
    s.push('<div>Save</div>');
    s.push('<img class="dn" width="20" height="20" src="../images/hourglass.gif" />');
    s.push('</button>');
...
Reader.prototype = {
    ...
    forums_inner_print: function (group_id, page, clickFromPager, reLoad) {
        ...
        if (!len) {
            ...
            for (i = 0; i < len; i++) {
                ...
                if (item.http_status < 400) {
                    add_class += ' forum-status-ok';
                }
                if (item.http_status >= 400 && item.http_status < 500) {
                    add_class += ' forum-status-not-found';
                }
                if (item.http_status >= 500) {
                    add_class += ' forum-status-error';
                }
                ...
                s.push('<p class="forum-feed-http-status" title="http-status">' + item.http_status + '</p>');
                s.push('<p class="forum-feed-last-item-date" title="lastItemDate: ' + this.date_format_simply(item.lastItemDate, 1, 1) + '">' + this.date_format_simply(item.lastItemDate, 0) + '</p>');
                s.push('<p class="forum-feed-description" title="' + that.escape(item.description) + '">' + item.description + '</p>');
                s.push('<a href="/forum/' + item.primaryKey + '/edit/" class="forum-edit-action" title="Edit subscription"></a>');
        ...
        $('.forum-forums > li a.forum-edit-action', root)
            .unbind('click').bind('click', function (event) {
                var el = $(this);
                var elRoot = el.parent();
                ...
                if (j != -1) {
                    ...
                    if (j != -1) {
                        ...
                        let field = !c['link_301'] ? 'link' : 'link_301';
                        let link = c[field];

                        Boxy.inputValue('Edit url of RSS:', function (value) {
                            if (!value) return false;

                            let vset = { "primaryKey": c['primaryKey'] };
                            vset[field] = value;

                            that.func_sendMessage("feeds_edit", 0, { "item": vset }, function (item) {
                                that.subs_update();
                            });
    ...
    item_dates_print: function (item) {
        ...
        s.push('<abbr title="Published: ' + this.date_format_simply(item.datetime, 1, 1) + '" class="published">');
        s.push(this.date_format_simply(item.datetime));
        s.push('</abbr>');
        s.push('<br />');

        if (item.datetime != item.time_load) {
            s.push('<abbr title="Recieved: ' + this.date_format_simply(item.time_load, 1, 1) + '" class="recieved">');
    ...
    date_format_simply: function (time, print_time, print_seconds) {
        if (typeof print_time == 'undefined') print_time = true;
        ...
        var df = day + '.' + month + '.' + d.getFullYear();
        df += (!print_time ? '' : ' ' + hours + ':' + minutes);
        df += (!print_seconds ? '' : ':' + seconds);
